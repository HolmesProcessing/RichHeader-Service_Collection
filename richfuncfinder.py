...
#
#by Timo 'T31M' Geissler & Julian 'kirschju' Kirsch
					...
					for p in sig['raw']:
						...
						if (found == len(sig['raw'])):
							...
							if virtAddr not in [func['virtAddr'] for func in found_functions]:
								found_functions.append({'virtAddr': virtAddr, 'name': sig['name'].decode("utf-8")})
					...
					for p in sig['raw']:
						...
						if (found == len(sig['raw'])):
							...
							if virtAddr not in [func['virtAddr'] for func in found_dict['functions']]:
								found_dict['functions'].append({'virtAddr': virtAddr, 'name': sig['name'].decode("utf-8")})
